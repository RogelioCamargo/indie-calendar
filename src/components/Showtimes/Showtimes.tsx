...
const Showtimes = () => {
	...
	const morningScreenings = screenings.filter((screening: ScreeningFull) =>
		screening.time.toUpperCase().includes("AM")
	);
	const eveningScreenings = screenings.filter((screening: ScreeningFull) =>
			...
			{screenings.map((screening: ScreeningFull) => (
				<div key={screening.id} id={screening.id}>
					<div className="px-7 mb-7 md:mb-12 md:flex md:max-w-7xl md:mx-auto">
						<img
							className="block w-full h-full bg-gray-300 md:max-w-sm md:mr-5"
